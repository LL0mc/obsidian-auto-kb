...
**所有页面模板、frontmatter 规范、目录结构以 `.opencode/skills/llm-wiki/SKILL.md` 为准。** 本文件只记录本项目的差异和扩展。
...
写入 `kb/wiki/sources/summary-{slug}.md`。**页面模板见 `llm-wiki/SKILL.md` 的 Page Template。**
...
更新 `kb/wiki/index.md`（格式见 `llm-wiki/SKILL.md` 的 Special Files）。

### Step 5: 追加日志

追加到 `kb/wiki/log.md`（格式见 `llm-wiki/SKILL.md` 的 Special Files）。
