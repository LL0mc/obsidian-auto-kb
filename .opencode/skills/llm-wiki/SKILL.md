...
> **适配说明：** 本项目 vault 的 wiki 文件不在根目录，而在 `kb/wiki/` 子目录下。
> 设置 `WIKI_DIR=kb/wiki` 后，所有 `index.md`/`log.md` 引用解析为 `$OBSIDIAN_VAULT_PATH/kb/wiki/index.md` 等。
...
Every wiki has these files at `$OBSIDIAN_VAULT_PATH/kb/wiki/`:
...
|---|---|---|
| Does a page exist? What's its title/category/tags? | Read `kb/wiki/index.md`; `Grep` frontmatter blocks under `kb/wiki/` (scope with a pattern that targets `^---` blocks at file heads) | **Cheapest** |
